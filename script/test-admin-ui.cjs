...
async function context(role = 'admin') {
  ...
  const state = { posts: [], adminGets: [], failNext: false, failLoad: false, deny: false, failLogout: false, providers: [structuredClone(provider)], suppliers: [structuredClone(supplier)], doctors: [structuredClone(doctor)], city: [structuredClone(city)] };
  ...
  await ctx.route('**/*', async route => {
    ...
    if (p === '/api/auth/logout' && state.failLogout) return route.fulfill({ status: 503, contentType: 'application/json', body: '{"message":"Тестовая ошибка выхода"}' });
...
(async () => {
  try {
    ...
    await check('read errors expose retry, not fake zero counts', async () => { state.failLoad=true; await visit(page,'overview'); await page.getByRole('alert').filter({hasText:'Тестовая ошибка сервера'}).waitFor(); assert.equal(await page.locator('.admin-stats').count(),0); state.failLoad=false; await page.getByRole('button',{name:'Повторить',exact:true}).click(); await page.locator('.admin-stats').waitFor(); });
    await check('failed logout does not pretend the server session ended', async () => { state.failLogout=true; await page.getByRole('button',{name:'Выйти из аккаунта',exact:true}).click(); await page.getByRole('alert').filter({hasText:'Не удалось завершить сессию'}).waitFor(); await page.getByRole('heading',{name:'Обзор',exact:true}).waitFor(); state.failLogout=false; });
