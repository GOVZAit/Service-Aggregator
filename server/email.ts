...
const emailFrom = process.env.EMAIL_FROM || "GOVZA pro <onboarding@resend.dev>";
...
export async function sendWelcomeEmail(message: WelcomeEmail): Promise<boolean> {
  ...
  return deliverEmail(
    message.to,
    "Аккаунт GOVZA pro создан",
    `<p>Ваш аккаунт в сервисе GOVZA pro успешно создан.</p>
...
export async function sendPasswordResetEmail(message: PasswordResetEmail): Promise<boolean> {
  ...
  return deliverEmail(
    message.to,
    "Восстановление доступа к GOVZA pro",
