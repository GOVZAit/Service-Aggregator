...
import { Clock, MapPin, CheckCircle2, XCircle, PhoneCall, Mail, MessageSquare } from "lucide-react";
...
export default function MasterOrdersPage() {
  ...
  return <div className="app-page bg-background">
    <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border/60 safe-area-pt"><div className="max-w-lg lg:max-w-4xl mx-auto px-4 pt-4"><h1 className="text-xl font-bold mb-3">Заказы</h1><div className="flex gap-1 pb-3">{tabs.map(({ key, label }) => <button key={key} onClick={() => setActiveTab(key)} data-testid={`tab-orders-${key}`} className={cn("flex-1 min-h-[44px] flex justify-center items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium", activeTab === key ? "bg-primary text-white" : "bg-muted text-muted-foreground")}>{label}<span className={cn("text-xs font-bold px-1.5 py-0.5 rounded-full", activeTab === key ? "bg-white/20" : "bg-background")}>{groups[key].length}</span></button>)}</div></div></header>
    <main className="max-w-lg lg:max-w-4xl mx-auto px-4 py-4">
      {isLoading ? <div className="space-y-3 py-2">{[1,2,3].map((item) => <div key={item} className="h-40 rounded-2xl bg-muted animate-pulse" />)}</div> : current.length === 0 ? <div className="rounded-2xl border bg-card py-12 text-center"><p className="font-medium">Здесь пока пусто</p><p className="text-sm text-muted-foreground mt-1">Заказы автоматически распределяются по статусам</p></div> : <div className="grid lg:grid-cols-2 gap-3">{current.map((order) => <article key={order.id} className="rounded-2xl bg-card border border-border/60 p-4 space-y-3" data-testid={`master-order-${order.id}`}>
        ...
        {order.clientContact && activeTab !== "done" && <a href={contactLink(order.clientContact)} className="flex items-center justify-center gap-2 h-11 rounded-xl border border-primary/30 text-primary text-sm font-semibold">{order.clientContact.includes("@") ? <Mail className="w-4 h-4" /> : <PhoneCall className="w-4 h-4" />}{order.clientContact}</a>}
        {order.status === "pending" && <div className="flex gap-2"><button disabled={mutation.isPending} onClick={() => mutation.mutate({ id: order.id, status: "rejected" })} className="flex-1 h-11 rounded-xl border text-sm font-semibold text-muted-foreground"><XCircle className="w-4 h-4 inline mr-1" />Отклонить</button><button disabled={mutation.isPending} onClick={() => mutation.mutate({ id: order.id, status: "in_progress" })} className="flex-[2] h-11 rounded-xl bg-primary text-white text-sm font-semibold"><CheckCircle2 className="w-4 h-4 inline mr-1" />Принять</button></div>}
        {order.status === "in_progress" && <button disabled={mutation.isPending} onClick={() => mutation.mutate({ id: order.id, status: "completed" })} className="w-full h-11 rounded-xl bg-green-600 text-white text-sm font-semibold"><CheckCircle2 className="w-4 h-4 inline mr-1" />Завершить заказ</button>}
