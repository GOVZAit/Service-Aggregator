...
export default function MasterProfilePage() {
  ...
  return <div className="app-page bg-background">
    <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b px-4 py-3 safe-area-pt"><div className="max-w-lg lg:max-w-4xl mx-auto flex justify-between items-center"><div><h1 className="text-xl font-bold">Профиль мастера</h1><p className="text-xs text-muted-foreground">Изменения видны клиентам</p></div>{message && <span className={cn("text-xs font-medium", message === "Сохранено" ? "text-green-600" : "text-destructive")}>{message}</span>}</div></header>
    <main className="max-w-lg lg:max-w-4xl mx-auto px-4 py-4 space-y-4">
      <section className="rounded-2xl bg-card border p-4 flex items-center gap-4"><div className="relative">{master.avatar ? <img src={master.avatar} alt={master.name} className="w-20 h-20 rounded-2xl object-cover" /> : <div className="w-20 h-20 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-bold text-2xl">{initials}</div>}<label className="absolute -right-2 -bottom-2 w-11 h-11 rounded-full bg-primary text-white flex items-center justify-center cursor-pointer"><Camera className="w-4 h-4" /><input type="file" accept="image/*" className="hidden" onChange={(event) => readImage(event.target.files?.[0], "avatar")} /></label></div><div><h2 className="font-bold text-lg">{master.name}</h2><p className="text-sm text-muted-foreground">{master.category} · {master.city}</p><p className="text-xs mt-1">★ {master.rating} · {master.completedOrders} выполнено</p></div></section>

      <section className="rounded-2xl bg-card border p-4 space-y-3"><h3 className="font-semibold flex items-center gap-2"><User className="w-4 h-4 text-primary" />Основная информация</h3><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} maxLength={1000} placeholder="Расскажите об опыте и гарантиях" className="w-full rounded-xl bg-muted border px-3 py-3 text-base resize-none" data-testid="input-description" /><div className="grid sm:grid-cols-2 gap-2"><div className="relative"><Building2 className="absolute left-3 top-4 w-4 h-4 text-muted-foreground" /><Input autoComplete="organization" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Компания или бренд" className="h-12 pl-9" /></div><div className="relative"><Phone className="absolute left-3 top-4 w-4 h-4 text-muted-foreground" /><Input inputMode="tel" autoComplete="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Телефон для клиентов" className="h-12 pl-9" /></div></div><div className="grid grid-cols-2 gap-2"><select value={category} onChange={(e) => setCategory(e.target.value)} className="h-12 rounded-xl border bg-background px-3 text-base">{categories.map((item) => <option key={item.id}>{item.name}</option>)}</select><select value={city} onChange={(e) => setCity(e.target.value)} className="h-12 rounded-xl border bg-background px-3 text-base">{cities.filter((item) => item !== "Все города").map((item) => <option key={item}>{item}</option>)}</select></div><div><p className="text-xs font-medium text-muted-foreground mb-2">Приём звонков</p><div className="grid grid-cols-2 gap-2">{callModes.map((mode) => <button key={mode.id} onClick={() => setCallMode(mode.id)} className={cn("pressable min-h-[44px] rounded-xl border px-3 py-2 text-xs font-medium", callMode === mode.id && "border-primary bg-primary/5 text-primary")}>{mode.label}</button>)}</div></div>{callMode === "schedule" && <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-primary" /><select value={workFrom} onChange={(e) => setWorkFrom(e.target.value)} className="h-11 rounded-lg border bg-background px-2">{hours.map((hour) => <option key={hour}>{hour}</option>)}</select><span>—</span><select value={workTo} onChange={(e) => setWorkTo(e.target.value)} className="h-11 rounded-lg border bg-background px-2">{hours.map((hour) => <option key={hour}>{hour}</option>)}</select></div>}<Button onClick={saveMain} disabled={mutation.isPending || description.trim().length < 10} className="w-full h-12"><Save className="w-4 h-4 mr-2" />Сохранить профиль</Button></section>
