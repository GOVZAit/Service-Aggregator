    ...
    <div className="min-h-[100dvh] bg-background safe-area-pt">
      <main className="mx-auto max-w-lg space-y-6 px-4 py-6">
        ...
        <div className="pt-4 text-center">
          ...
          <h1 className="mt-4 text-3xl font-extrabold tracking-[-0.04em]">Настройте организацию</h1>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(Object.entries(organizationKindLabels) as Array<[OrganizationKind, string]>).map(([value, label]) => (
              <button
                ...
                className={cn(
                  "min-h-11 rounded-2xl border border-border/70 px-3 py-2 text-left text-sm",
            ...
            {categories.map((category) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex min-h-12 items-center justify-between rounded-2xl border border-border/70 px-3 text-sm font-medium",
        ...
        <Button
          className="accent-gradient h-12 w-full rounded-2xl font-bold text-white"
